...
function startAndEndWith(letter) {
  let arr = [];
  for (let i = 0; i < words.length; i++) {
    if (words[i].charAt(0).toLowerCase() == letter && words[i].charAt(words[i].length - 1).toLowerCase() == letter) {
      arr.push(words[i]);
    }
  }
  return arr;
...
function countMoney(coins) {
  let coinCount = 0;
  let q = (coins["quarters"]) * 25;
  let d = (coins["dimes"]) * 10;
  let n = (coins["nickels"]) * 5;
  let p = (coins["pennies"]) * 1;
  coinCount = q + d + n + p;
...
function waitFor24() {
  let num24 = readline.question("Enter in a number: ");
  while (num24 != 24) {
    num24 = readline.question("Enter in a number: ");
  }
}
